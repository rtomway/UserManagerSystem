...
QWidget* LoginRegisterDlg::CreateLoginWidget()
{
	...
	m_accountEdit->setPlaceholderText("输入用户名/手机/邮箱");
	...
	connect(m_autoLoginChx, &QCheckBox::stateChanged, [=](int checked) 
		{
			...
			config->setValue("user/autoLogin", checked);
		});
...
QWidget* LoginRegisterDlg::CreateRegisterWidget()
{
	...
	m_regit_accountEdit->setPlaceholderText("请输入用户名/手机号/邮箱");
	m_regit_passwordEdit->setPlaceholderText("请输入密码");
	m_regit_rePasswordEdit->setPlaceholderText("请再次输入密码");
	m_regit_nicknameEdit->setPlaceholderText("请输入网名");
...
void LoginRegisterDlg::onLogin()
{
	...
	SHttpClient(URL("/api/login")).debug(true)
		...
		.fail([=](const QString& msg, int code)
			{
				...
				{ 
					QMessageBox::warning(this, "登录失败", "服务器连接失败");
				}
		...
		.success([=](const QByteArray& data)
			{
				...
				{
					auto token = jdom.object().value("data").toObject().value("token").toString();
...
void LoginRegisterDlg::paintEvent(QPaintEvent* ev)
{
	...
	painter.drawStaticText(48, 120, QStaticText("  黄河管理系统——客户端 v2.1.0"));
